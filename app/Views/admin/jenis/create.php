...
<head>
    ...
    <title>Tambah Jenis Produk - Kasir Bangunan</title>
...
<body>
    ...
    <div class="main-content">
        <nav class="top-navbar">
            <div class="page-title"><i class="fas fa-plus-circle me-2"></i> Tambah Jenis Produk</div>
        ...
        <div class="content-area">
            ...
            <div class="form-card">
                <h4 class="fw-bold mb-4"><i class="fas fa-tag me-2"></i> Form Tambah Jenis Produk</h4>
                
                <form action="<?= base_url('admin/jenis-produk/store') ?>" method="POST">
                    <?= csrf_field() ?>
                    
                    <div class="mb-3">
                        <label class="form-label">Kode Jenis</label>
                        <input type="text" name="kode_jenis" class="form-control <?= (session('validation') && session('validation')->hasError('kode_jenis')) ? 'is-invalid' : '' ?>" placeholder="Contoh: JNS001" required>
                        <?php if(session('validation') && session('validation')->hasError('kode_jenis')): ?>
                            <div class="invalid-feedback"><?= session('validation')->getError('kode_jenis') ?></div>
                        <?php endif; ?>
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label">Nama Jenis</label>
                        <input type="text" name="nama_jenis" class="form-control <?= (session('validation') && session('validation')->hasError('nama_jenis')) ? 'is-invalid' : '' ?>" placeholder="Contoh: Semen, Cat, Besi" required>
